...
func TestProxyRechargeShopReturnsInteractiveVerificationReason(t *testing.T) {
	t.Parallel()
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "/shopApi/Shop/getGoodsPrice", r.URL.Path)
...
<textarea id="renderData" style="display:none">var requestInfo = {"traceid":"`+traceID+`","sceneId":"19x5u7lo"};</textarea>`)
	...
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Params = gin.Params{{Key: "action", Value: "price"}}
	ctx.Request = httptest.NewRequest(
		http.MethodPost,
		"/api/v1/payment/recharge-shop/price",
		bytes.NewBufferString(`{"goods_key":"y8virm","quantity":1,"channel_id":1}`),
	)
	ctx.Request.Header.Set("Content-Type", "application/json")
	...
	require.Contains(t, recorder.Body.String(), "RECHARGE_SHOP_VERIFICATION_REQUIRED")
	require.Contains(t, recorder.Body.String(), traceID)
}
