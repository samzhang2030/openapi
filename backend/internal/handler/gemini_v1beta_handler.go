...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	// 检查平台：优先使用强制平台（/antigravity 路由），否则要求 gemini/mixed 分组
	forcePlatform, hasForcePlatform := middleware.GetForcePlatformFromContext(c)
	if !hasForcePlatform && !isGeminiNativeCompatibleGroup(apiKey.Group) {
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	// 检查平台：优先使用强制平台（/antigravity 路由，中间件已设置 request.Context），否则要求 gemini/mixed 分组
	if !middleware.HasForcePlatform(c) {
		if !isGeminiNativeCompatibleGroup(apiKey.Group) {
			googleError(c, http.StatusBadRequest, "API key group platform is not gemini")
			return
		}
	}
...
func isGeminiNativeCompatibleGroup(group *service.Group) bool {
	return group != nil && (group.Platform == service.PlatformGemini || group.Platform == service.PlatformMixed)
}
