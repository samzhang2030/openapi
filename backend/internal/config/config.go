...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	cfg.Update.ExternalUpdaterCommand = strings.TrimSpace(cfg.Update.ExternalUpdaterCommand)
	cfg.Update.ExternalUpdaterWorkingDirectory = strings.TrimSpace(cfg.Update.ExternalUpdaterWorkingDirectory)
...
func bindEnvKeys() {
	for _, key := range []string{
		"update.proxy_url",
		"update.external_updater_command",
		"update.external_updater_working_directory",
		"update.external_updater_helper_timeout_seconds",
	} {
		_ = viper.BindEnv(key)
	}
}
