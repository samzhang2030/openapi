...
func TestRateLimitService_HandleUpstreamError_Transient403CircuitBreakerBypassesThreshold(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	counter := &openAI403CounterCacheStub{counts: []int64{3}}
	service := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	service.SetOpenAI403CounterCache(counter)
	...
	shouldDisable := service.HandleUpstreamError(
		...
		http.StatusForbidden,
		http.Header{},
		[]byte(`{"error":{"message":"API Key 熔断已开启，请稍后重试"}}`),
	)

	require.True(t, shouldDisable)
	require.Equal(t, 0, repo.setErrorCalls)
	require.Equal(t, 1, repo.tempCalls)
	require.Contains(t, repo.lastTempReason, "API Key 熔断已开启")
	require.Len(t, counter.counts, 1, "transient circuit breaker 403 should not consume permanent-disable counter")
}
