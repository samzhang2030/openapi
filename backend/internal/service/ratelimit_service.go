...
func (s *RateLimitService) handle403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	...
	if isTransientForbiddenCircuitBreaker(upstreamMsg, responseBody) {
		rule := TempUnschedulableRule{
			ErrorCode:       http.StatusForbidden,
			DurationMinutes: openAI403CooldownMinutesDefault,
			Description:     "transient upstream circuit breaker",
		}
		if s.triggerTempUnschedulable(ctx, account, rule, -1, http.StatusForbidden, "circuit_breaker", responseBody) {
			return true
		}
	}
...
func isTransientForbiddenCircuitBreaker(upstreamMsg string, responseBody []byte) bool {
	text := strings.ToLower(strings.TrimSpace(upstreamMsg + "\n" + string(responseBody)))
	if text == "" {
		return false
	}
	if strings.Contains(text, "熔断") {
		return true
	}
	return strings.Contains(text, "circuit") && strings.Contains(text, "breaker")
}
