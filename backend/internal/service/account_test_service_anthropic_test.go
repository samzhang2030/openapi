...
func TestAccountTestService_ClaudeAPIKeyTestSetsSessionHeader(t *testing.T) {
	...
	require.Equal(t, "https://www.right.codes/v1/messages?beta=true", upstream.lastReq.URL.String())
...
func TestAccountTestService_ClaudeAPIKeyTestRightCodesPageURLUsesRelayRoot(t *testing.T) {
	c, _ := newTestContext()
	resp := newJSONResponse(http.StatusOK, "data: {\"type\":\"message_stop\"}\n\n")
	upstream := &anthropicHTTPUpstreamRecorder{resp: resp}
	svc := &AccountTestService{
		...
		cfg: &config.Config{
			Security: config.SecurityConfig{
				URLAllowlist: config.URLAllowlistConfig{
					Enabled:       true,
					UpstreamHosts: []string{"right.codes", "*.right.codes"},
				},
			},
		},
	}
