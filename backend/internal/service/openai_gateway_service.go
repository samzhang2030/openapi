...
func isOpenAICompatibleAccountEligibleForRequest(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	...
	if account == nil || account.Platform != platform || !account.IsSchedulableForModelWithContext(ctx, requestedModel) {
		return false
	}
	...
	if !accountSupportsOpenAICompatibleEndpointCapability(account, platform, requiredCapability) {
		return false
	}
...
func accountSupportsOpenAICompatibleEndpointCapability(account *Account, platform string, requiredCapability OpenAIEndpointCapability) bool {
	if account == nil {
		return false
	}
	...
	switch normalizeOpenAICompatibleAccountPlatform(platform) {
	case PlatformDeepSeek:
		return account.Platform == PlatformDeepSeek && requiredCapability == OpenAIEndpointCapabilityChatCompletions
	default:
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwarenessForPlatform(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	return s.selectAccountWithLoadAwareness(s.withOpenAIQuotaAutoPauseContext(ctx), groupID, platform, sessionHash, requestedModel, excludedIDs, false, "")
}
