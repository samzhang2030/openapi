...
func TestVisibleMethodLoadBalancerUsesLdxPayBridgeForAlipay(t *testing.T) {
	t.Parallel()
	...
	_, err := client.PaymentProviderInstance.Create().
		...
		Save(ctx)
	if err != nil {
		t.Fatalf("create ldxpaybridge provider: %v", err)
	}
	...
	_, err = lb.SelectInstance(ctx, "", payment.TypeAlipay, payment.StrategyRoundRobin, 29.9)
	if err != nil {
		t.Fatalf("SelectInstance returned error: %v", err)
	}
	if inner.lastProviderKey != payment.TypeLdxPayBridge {
		t.Fatalf("lastProviderKey = %q, want %q", inner.lastProviderKey, payment.TypeLdxPayBridge)
	}
}
